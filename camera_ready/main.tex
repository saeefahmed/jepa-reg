...
%%   2. Replace the repository URL placeholder in Sec. 4.2 and the Reproducibility statement.
%%   3. Replace rights/DOI/ISBN commands with the values from your ACM rights form.
...
Table~\ref{tab:hyper} lists all hyperparameters. Code, the analysis notebook, and per-seed results JSONs are released at \url{https://github.com/USERNAME/jepa-reg}.\footnote{TODO: replace with the actual repository URL.} BERT-base numbers come from a full rerun of the released pipeline; RoBERTa-base numbers from the original run of the same code (per-seed values for both are in the released JSONs).
...
Code, the analysis notebook (including all revision experiments), and per-seed results JSONs are released at \url{https://github.com/USERNAME/jepa-reg} (TODO: replace URL). Hardware: 1$\times$ NVIDIA RTX 5060 Ti (16 GB). Known asymmetry: label smoothing 0.05 (Baseline/SimCSE) vs.\ 0.0 (JEPA-Reg classification loss). Statistics: per-seed values in Tables~\ref{tab:bert}--\ref{tab:roberta}; the exact permutation floor $p=0.25$ at $n=3$ is stated in Section~\ref{sec:implementation}.
