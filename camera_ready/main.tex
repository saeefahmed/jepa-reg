...
Table~\ref{tab:hyper} lists all hyperparameters. Code, the analysis notebook, and per-seed results JSONs are released at \url{https://github.com/saeefahmed/jepa-reg}; the full software environment and reproducibility details are given in Appendix~\ref{app:repro}. BERT-base numbers come from a full rerun of the released pipeline; RoBERTa-base numbers from the original run of the same code (per-seed values for both are in the released JSONs).
